...
interface SearchSelectProps {
  ...
  value?: string;  // Esta propriedade representa o nome do time atualmente selecionado.
  initialValue?: string;
...
const SearchSelectTeam = ({ onSelectItem, value }: SearchSelectProps) => {
  ...
  useEffect(() => {
    if (initialValue) {
      const loadInitialItem = async () => {
        ...
        const q = query(playersCollection, where('name', '==', initialValue));
      ...
      {searchResults.length > 0 && searchText.length > 0 ? (
        <ul className={styles.resultsList}>
          {searchResults.map((item) => (
            <li
              key={item.id}
              className={`${styles.resultItem} ${isItemSelected(item) ? styles.resultItemSelected : ''
                }`}
